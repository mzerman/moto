...
class BasePage():
    ...
    def make_element_visible_by_js(self, locator_css):
        js = """const data_options = Array.from(document.querySelectorAll('{css}'));
                data_options.forEach(a=>{{a.style='display:block;';}});""".format(css=locator_css)
        self.driver.execute_script(js)

    def make_element_unvisible_by_js(self, locator_css):
        js = """const data_options = Array.from(document.querySelectorAll('{css}'));
                data_options.forEach(a=>{{a.style='display:none;';}});""".format(css=locator_css)
        self.driver.execute_script(js)
